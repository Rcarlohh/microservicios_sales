...
async def startup_event():
    ...
    logger.info(f"- POST {public_ip}:8000/ventas/ (Crear Venta)")
    logger.info(f"- GET {public_ip}:8000/ventas/ (Listar Ventas)")
    logger.info(f"- GET {public_ip}:8000/ventas/{{venta_id}} (Obtener Venta Específica)")
    logger.info(f"- GET {public_ip}:8000/ventas/periodo/ (Obtener Ventas por Periodo)")
